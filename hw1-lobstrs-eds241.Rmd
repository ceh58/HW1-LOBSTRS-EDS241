...
*The Isla Vista research site appears to be larger than the other sites, which can influence abundance measures if not properly accounted for. Additionally, the Naples and Isla Vista sites are closer together, potentially introducing some bias in location/habitat similarity as compared to the control group.*
...
ggplot(aes(x = counts, y = site)) +
    ...
    labs(x = "Counts",
         y = "Site",
         title = "Spiny Lobster Counts by Site",
         fill = "Site") +
    theme_minimal()
...
# plot 4:
beeswarm(spiny_counts$mean_size ~ spiny_counts$year,
         pch = 19,
         pwcol = factor(spiny_counts$mpa),
         xlab = "Year",
         ylab = "Mean Size",
         main = "Mean Size by Year (and MPA Status)") +
...
*The four diagnostic plots are showing us that are data are not normal (right-skewed). Therefore, we are violating an assumption of OLS and can deduce that the model might not be the best fit.*
...
*Overdispersion was detected, meaning variance is significantly larger than the mean. This violates the poisson assumption that variance is proportional to the mean, so a poisson model may not be the best fit for the data. Zero-inflation is probable...* 
...
*A negative binomial model was fit to account for the overdispersion detected in the poisson model.The treatment estiamte result is the same, indicating a 23% increase in lobster counts between non-MPA and MPA.*
------------------------------------------------------------------------
...
plot_counts %>% 
    ...
    labs(
        x = "Year",
        y = "Mean Counts",
        title = "Mean Lobster Counts by Year and Treatment Group",
        color = "Treatment"
    ) +
    theme_minimal()
